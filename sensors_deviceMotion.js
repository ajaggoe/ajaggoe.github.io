...
permbutton.addEventListener('click', requestPermission(DeviceMotionEvent))
  ...
  if(requestGrant){
        ...
        if (window.DeviceMotionEvent) {
          window.addEventListener('devicemotion', (event) => {
            this.document.getElementById("acc-info").innerHTML = `x: ${event.acceleration.x.toFixed(2)} <br>y: ${event.acceleration.y.toFixed(2)}<br>alpha: ${event.alpha}`
          });
        } else {
    ...
    this.document.getElementById("acc-info").innerHTML = `No access motherfoeker`
  }
}
